<script lang="ts">
	...
	const insertText = (word: string, enableNewline: boolean = true): void => {
		if (editCommentElement === undefined) {
			return;
		}
		let sentence = editCommentElement.value;
		const len = sentence.length;
		const pos = editCommentElement.selectionStart;
		...
		editCommentElement.value = sentence;
		editCommentElement.focus();
		editCommentElement.selectionStart = pos + word.length;
		editCommentElement.selectionEnd = pos + word.length;
		editCommentValue = sentence;
	};
	const callGetEmoji = () => {
...
<div class="comment-form">
	{#if isContentWarningEnabled}
		...
		/>
	{/if}
	<textarea class="comment" bind:value={editCommentValue} bind:this={editCommentElement}></textarea>
	<span class="add-cw">
		<button
			type="button"
			class="svg add-cw"
	...
	<button
		type="button"
		class="send-comment"
		disabled={editCommentValue.length === 0}
		onclick={async () => {
			...
			const content = editCommentValue;
			editCommentValue = '';
