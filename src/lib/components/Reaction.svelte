...
>
	<span class="reactionstar-content">
		...
		{/if}
	</span>{#if profile !== undefined}
		{@const npub = nip19.npubEncode(reactionEvent.pubkey)}
		{@const name = profile.name ?? ''}<a class="reactionstar-link" href="/{npub}">
			<img
				class="reactionstar-profile-picture"
				src={URL.canParse(profile.picture ?? '')
					? profile.picture
